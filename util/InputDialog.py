...
from PyQt5.QtWidgets import (QDialog, QFormLayout, QLineEdit, QDialogButtonBox, QInputDialog, QMessageBox, QComboBox,
                             ...
                             QFileDialog, QColorDialog, QPushButton, QTextEdit, QCheckBox)
...
image_dir = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
...
class QDataSelection2(QDialog):
    def __init__(self, Protein=None, DNA=None, RNA=None, Structure=None, Chemical=None):
        super(QDataSelection2, self).__init__()
        self.setStyleSheet(qdarkstyle.load_stylesheet_pyqt5())
        self.setWindowIcon(QIcon('images/logo.ico'))        
        self.Protein = Protein[1:, 1:].astype(float) if Protein is not None else None
        self.DNA = DNA[1:, 1:].astype(float) if DNA is not None else None
        self.RNA = RNA[1:, 1:].astype(float) if RNA is not None else None
        self.Structure = Structure.values[:, 1:].astype(float) if Structure is not None else None
        self.Chemical = Chemical.values[:, 1:].astype(float) if Chemical is not None else None        
        self.data_source = None
        self.initUI()
    ...
    def initUI(self):
        ...
        self.dataTreeWidget.clicked.connect(self.treeClicked)
        self.dataTreeWidget.setHeaderLabels(['Source', 'Shape'])
        if self.Protein is not None and len(self.Protein) > 0:
            protein_descriptor = QTreeWidgetItem(self.dataTreeWidget)
            protein_descriptor.setText(0, 'Protein descriptor data')
            protein_descriptor.setText(1, '(%s, %s)' % (self.Protein.shape[0], self.Protein.shape[1]))
        if self.DNA is not None and len(self.DNA) > 0:
            dna_descriptor = QTreeWidgetItem(self.dataTreeWidget)
            dna_descriptor.setText(0, 'DNA descriptor data')
            dna_descriptor.setText(1, '(%s, %s)' % (self.DNA.shape[0], self.DNA.shape[1]))
        if self.RNA is not None and len(self.RNA) > 0:
            rna_descriptor = QTreeWidgetItem(self.dataTreeWidget)
            rna_descriptor.setText(0, 'RNA descriptor data')
            rna_descriptor.setText(1, '(%s, %s)' % (self.RNA.shape[0], self.RNA.shape[1]))
        if self.Structure is not None and len(self.Structure) > 0:
            structure_descriptor = QTreeWidgetItem(self.dataTreeWidget)
            structure_descriptor.setText(0, 'Structure descriptor data')
            structure_descriptor.setText(1, '(%s, %s)' % (self.Structure.shape[0], self.Structure.shape[1]))
        if self.Chemical is not None and len(self.Chemical) > 0:
            chemical_descriptor = QTreeWidgetItem(self.dataTreeWidget)
            chemical_descriptor.setText(0, 'Chemical descriptor data')
            chemical_descriptor.setText(1, '(%s, %s)' % (self.Chemical.shape[0], self.Chemical.shape[1]))
        self.buttons = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel, Qt.Horizontal, self)
        self.buttons.accepted.connect(self.accept)
        self.buttons.rejected.connect(self.reject)
        layout.addWidget(self.dataTreeWidget)
    ...
    def treeClicked(self, index):
        item = self.dataTreeWidget.currentItem()
        if item and item.text(0) in ['Protein descriptor data', 'DNA descriptor data', 'RNA descriptor data', 'Structure descriptor data', 'Chemical descriptor data']:
            self.data_source = item.text(0)
    ...
    @staticmethod
    def getValues(Protein=None, DNA=None, RNA=None, Structure=None, Chemical=None):
        dialog = QDataSelection2(Protein, DNA, RNA, Structure, Chemical)
        result = dialog.exec_()
        data_source = dialog.getDataSource()
        return data_source, result == QDialog.Accepted
...
class QStructureResidueDialog(QDialog):
    def __init__(self):
        super(QStructureResidueDialog, self).__init__()
        self.setStyleSheet(qdarkstyle.load_stylesheet_pyqt5())
        self.setWindowIcon(QIcon(os.path.join(image_dir, 'images', 'logo.ico')))
        self.initUI()
    
    def initUI(self):
        self.setWindowFlags(Qt.WindowCloseButtonHint)
        self.setWindowTitle('Structure descriptor setting')
        self.resize(400, 110)
        self.setFont(QFont('Arial', 10))
        layout = QFormLayout(self)
        self.residue_shell_from = MyLineEdit("3")
        self.residue_shell_from.clicked.connect(self.setShellFromValue)
        self.residue_shell_to = MyLineEdit("20")
        self.residue_shell_to.clicked.connect(self.setShellToValue)
        self.residue_shell_step = MyLineEdit("3")
        self.residue_shell_step.clicked.connect(self.setShellStepValue)
        self.target_residues = QTextEdit()
        self.target_residues.setFont(QFont('Arial', 8))        
        layout.addRow('Shell from (Å):', self.residue_shell_from)
        layout.addRow('Shell to (Å):', self.residue_shell_to)        
        layout.addRow('Shell step (Å):', self.residue_shell_step)
        # layout.addRow('Target residues:', QLabel(''))
        # layout.addRow(self.target_residues)
        self.buttons = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel, Qt.Horizontal, self)
        self.buttons.accepted.connect(self.accept)
        self.buttons.rejected.connect(self.reject)
        layout.addWidget(self.buttons)

    def setShellFromValue(self):
        num, ok, = QInputDialog.getInt(self, 'Shell setting', 'Set shell begin value', 3, 1, 10, 1)
        if ok:
            self.residue_shell_from.setText(str(num))
    
    def setShellToValue(self):
        num, ok, = QInputDialog.getInt(self, 'Shell setting', 'Set shell end value', 20, 1, 50, 1)
        if ok:
            self.residue_shell_to.setText(str(num))
    
    def setShellStepValue(self):
        num, ok, = QInputDialog.getInt(self, 'Shell setting', 'Set shell step value', 3, 1, 10, 1)
        if ok:
            self.residue_shell_step.setText(str(num))

    def getFromValue(self):
        return int(self.residue_shell_from.text()) if self.residue_shell_from.text() != '' and 0<int(self.residue_shell_from.text())<=10 else 3

    def getToValue(self):
        return int(self.residue_shell_to.text()) if self.residue_shell_to.text() != '' and 0<int(self.residue_shell_to.text())<=50 else 20

    def getStepValue(self):
        return int(self.residue_shell_step.text()) if self.residue_shell_step.text() != '' and 0<int(self.residue_shell_step.text())<=10 else 3

    # def getTargetResidues(self):
    #     return self.target_residues.toPlainText()

    @staticmethod
    def getValues():
        dialog = QStructureResidueDialog()
        result = dialog.exec_()
        begin = dialog.getFromValue()
        to = dialog.getToValue()
        step = dialog.getStepValue()        
        return (begin, to, step),  result == QDialog.Accepted


class QStructureAtomDialog(QDialog):
    def __init__(self):
        super(QStructureAtomDialog, self).__init__()
        self.setStyleSheet(qdarkstyle.load_stylesheet_pyqt5())
        self.setWindowIcon(QIcon(os.path.join(image_dir, 'images', 'logo.ico')))
        self.initUI()
    ...
    def initUI(self):
        ...
        self.setFont(QFont('Arial', 10))
        layout = QFormLayout(self)
        self.residue_shell_from = MyLineEdit("1")
        self.residue_shell_from.clicked.connect(self.setShellFromValue)
        self.residue_shell_to = MyLineEdit("10")
        self.residue_shell_to.clicked.connect(self.setShellToValue)
        self.residue_shell_step = MyLineEdit("1")
        self.residue_shell_step.clicked.connect(self.setShellStepValue)
        self.target_residues = QTextEdit()
        self.target_residues.setFont(QFont('Arial', 8))        
        layout.addRow('Shell from (Å):', self.residue_shell_from)
    ...
    def setShellFromValue(self):
        num, ok, = QInputDialog.getInt(self, 'Shell setting', 'Set shell begin value', 1, 1, 10, 1)
        if ok:
            self.residue_shell_from.setText(str(num))
    
    def setShellToValue(self):
        num, ok, = QInputDialog.getInt(self, 'Shell setting', 'Set shell end value', 10, 1, 50, 1)
        if ok:
            self.residue_shell_to.setText(str(num))
    
    def setShellStepValue(self):
        num, ok, = QInputDialog.getInt(self, 'Shell setting', 'Set shell step value', 1, 1, 10, 1)
        if ok:
            self.residue_shell_step.setText(str(num))

    def getFromValue(self):
        return int(self.residue_shell_from.text()) if self.residue_shell_from.text() != '' and 0<int(self.residue_shell_from.text())<=10 else 1

    def getToValue(self):
        return int(self.residue_shell_to.text()) if self.residue_shell_to.text() != '' and 0<int(self.residue_shell_to.text())<=50 else 10

    def getStepValue(self):
        return int(self.residue_shell_step.text()) if self.residue_shell_step.text() != '' and 0<int(self.residue_shell_step.text())<=10 else 1
...
class QDRInput(QDialog):
    def __init__(self):
        super(QDRInput, self).__init__()
        self.setStyleSheet(qdarkstyle.load_stylesheet_pyqt5())
        self.setWindowIcon(QIcon('images/logo.ico'))
        self.initUI()
    ...
    def initUI(self):
        self.setWindowFlags(Qt.WindowCloseButtonHint)
        self.setWindowTitle('Dimensionality reduction setting')
        self.setFont(QFont('Arial', 10))
        self.resize(400, 110)
        layout = QFormLayout(self)
        self.n_components_lineEdit = MyLineEdit("2")
        self.n_components_lineEdit.clicked.connect(self.set_n_components)
        self.include_label_checkBox = QCheckBox('', self)
        self.include_label_checkBox.setChecked(False)
        self.include_label_checkBox.setToolTip("Column 1 will be taken as the sample labels if it is checked.")
        self.buttons = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel, Qt.Horizontal, self)
        self.buttons.accepted.connect(self.accept)
        self.buttons.rejected.connect(self.reject)
        layout.addRow("Number of components:", self.n_components_lineEdit)
        layout.addRow("Sample label column:", self.include_label_checkBox)        
        layout.addWidget(self.buttons)

    def set_n_components(self):
        num, ok = QInputDialog.getInt(self, 'Number of components', 'Set number of components', 2, 2, 10000, 1)
        if ok:
            self.n_components_lineEdit.setText(str(num))

    def get_n_components(self):
        return int(self.n_components_lineEdit.text()) if self.n_components_lineEdit.text() != '' else 2

    def get_state(self):
        return self.include_label_checkBox.isChecked()
    
    @staticmethod
    def getValues():
        dialog = QDRInput()
        result = dialog.exec_()
        n_components = dialog.get_n_components()
        sample_label_state = dialog.get_state()        
